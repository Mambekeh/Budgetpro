// Currency utility for consistent formatting across the app

// ========== EXACT COUNTRY LIST (From Settings.jsx) ==========
const COUNTRIES = [
...
const NON_EUROZONE_EUROPE = [
  'Sweden', 'Norway', 'Denmark', 'Switzerland', 'Poland', 
  'Czech Republic', 'Hungary', 'Romania', 'Bulgaria', 'Serbia',
  'Croatia', 'Bosnia and Herzegovina', 'Albania', 'North Macedonia',
  'Iceland', 'Montenegro', 'Kosovo', 'Andorra', 'Monaco',
...
// ZONE 3: WHITELISTED AFRICA
const WHITELISTED_AFRICA = [
  'South Africa', 'Nigeria', 'Kenya', 'Ghana', 'Botswana'
];

// ZONE 4: NON-WHITELISTED AFRICA
const NON_WHITELISTED_AFRICA = [
  'Angola', 'Zimbabwe', 'Tanzania', 'Uganda', 'Zambia',
  'Mozambique', 'Namibia', 'Lesotho', 'Eswatini', 'Rwanda',
  'Burundi', 'Ethiopia', 'Sudan', 'South Sudan', 'Somalia',
  'Djibouti', 'Eritrea', 'Egypt', 'Libya', 'Tunisia',
  'Algeria', 'Morocco', 'Mauritania', 'Senegal', 'Mali',
  'Burkina Faso', 'Guinea', 'Sierra Leone', 'Liberia',
  'Benin', 'Niger', 'Chad', 'Cameroon', 'Gabon', 'Congo',
  'Democratic Republic of the Congo', 'Madagascar', 'Mauritius',
  'Seychelles', 'Comoros', 'Cabo Verde', 'Sao Tome and Principe'
];
...
// ADD BACK: getCurrencyFromCountry function for Signup.jsx
export const getCurrencyFromCountry = (countryName) => {
  if (!countryName) return 'ZAR';
  
  const normalizedName = normalizeCountryName(countryName);
  if (!normalizedName) return 'ZAR';
  
  const country = COUNTRIES.find(c => c.name === normalizedName);
  return country ? country.currency : 'ZAR';
};
...
const normalizeCountryName = (countryName) => {
  ...
  // Try code match
  const codeMatch = COUNTRIES.find(c => 
  ...
  return partialMatch ? partialMatch.name : countryName;
};

const getCountryZone = (countryName) => {
  const normalizedName = normalizeCountryName(countryName);
  if (!normalizedName) return 'rest-of-world';
  
  // DEBUG
  console.log(`DEBUG: ${countryName} → ${normalizedName}`);
  
  // ZONE 1: EUROPE
  if (normalizedName === 'United Kingdom') return 'uk';
  if (EUROZONE.includes(normalizedName)) return 'eurozone';
  if (NON_EUROZONE_EUROPE.includes(normalizedName)) return 'non-eurozone-europe';
  
  // ZONE 2: USA & CANADA
  if (US_CANADA.includes(normalizedName)) return 'us-canada';
  
  // ZONE 3: WHITELISTED AFRICA
  if (WHITELISTED_AFRICA.includes(normalizedName)) return 'whitelisted-africa';
  
  // ZONE 4: NON-WHITELISTED AFRICA
  if (NON_WHITELISTED_AFRICA.includes(normalizedName)) return 'non-whitelisted-africa';
...
// ========== DASHBOARD FUNCTIONS ==========
export const shouldDisplayCurrencySymbol = (currencyCode) => {
  if (!currencyCode) return false;
  const whitelistedCurrencies = ['ZAR', 'USD', 'EUR', 'GBP', 'NGN', 'GHS', 'BWP'];
  return whitelistedCurrencies.includes(currencyCode);
};

export const getCurrencySymbol = (currency = null) => {
  const userCurrency = currency || getUserCurrency();
  if (shouldDisplayCurrencySymbol(userCurrency)) {
...
// ========== COUNTRY PRICING (EXACT MATCHING) ==========
export const getCountryPricing = (countryName) => {
  const zone = getCountryZone(countryName);
  
  console.log(`PRICING DEBUG: "${countryName}" → Zone: ${zone}`);
  
  switch(zone) {
    // ZONE 1: EUROPE
    case 'uk':
      return {
        basic: { price: '£3.99', amount: 399, currency: "GBP" },
        pro: { price: '£5.99', amount: 599, currency: "GBP" },
        lifetime: { price: '£149.99', amount: 14999, currency: "GBP" }
      };
    
    case 'eurozone':
    case 'non-eurozone-europe':
      return {
        basic: { price: '€3.99', amount: 399, currency: "EUR" },
        pro: { price: '€5.99', amount: 599, currency: "EUR" },
        lifetime: { price: '€149.99', amount: 14999, currency: "EUR" }
      };
    
    // ZONE 2: USA & CANADA
    case 'us-canada':
      return {
        basic: { price: '$3.99', amount: 399, currency: "USD" },
        pro: { price: '$5.99', amount: 599, currency: "USD" },
        lifetime: { price: '$149.99', amount: 14999, currency: "USD" }
      };
    
    // ZONE 3: WHITELISTED AFRICA
    case 'whitelisted-africa':
      const normalizedName = normalizeCountryName(countryName);
      
      if (normalizedName === 'South Africa') {
        return {
          basic: { price: 'R49', amount: 4900, currency: "ZAR" },
          pro: { price: 'R89', amount: 8900, currency: "ZAR" },
          lifetime: { price: 'R1,999', amount: 199900, currency: "ZAR" }
        };
      }
      
      if (normalizedName === 'Nigeria') {
        return {
          basic: { price: '₦2,999', amount: 299900, currency: "NGN" },
          pro: { price: '₦5,999', amount: 599900, currency: "NGN" },
          lifetime: { price: '₦199,999', amount: 19999900, currency: "NGN" }
        };
      }
      
      if (normalizedName === 'Kenya') {
        return {
          basic: { price: 'KSh499', amount: 49900, currency: "KES" },
          pro: { price: 'KSh999', amount: 99900, currency: "KES" },
          lifetime: { price: 'KSh27,999', amount: 2799900, currency: "KES" }
        };
      }
      
      if (normalizedName === 'Ghana') {
        return {
          basic: { price: 'GH₵35', amount: 3500, currency: "GHS" },
          pro: { price: 'GH₵55', amount: 5500, currency: "GHS" },
          lifetime: { price: 'GH₵1,899', amount: 189900, currency: "GHS" }
        };
      }
      
      if (normalizedName === 'Botswana') {
        return {
          basic: { price: 'P37', amount: 3700, currency: "BWP" },
          pro: { price: 'P67', amount: 6700, currency: "BWP" },
          lifetime: { price: 'P1,899', amount: 189900, currency: "BWP" }
        };
      }
      
      // Fallback
      return {
        basic: { price: '$2.99', amount: 299, currency: "USD" },
        pro: { price: '$3.99', amount: 399, currency: "USD" },
        lifetime: { price: '$99.99', amount: 9999, currency: "USD" }
      };
    
    // ZONE 4: NON-WHITELISTED AFRICA
    case 'non-whitelisted-africa':
      return {
        basic: { price: '$1.99', amount: 199, currency: "USD" },
        pro: { price: '$3.99', amount: 399, currency: "USD" },
        lifetime: { price: '$50', amount: 5000, currency: "USD" }
      };
...
export const getUserCurrency = () => {
  return localStorage.getItem('bp_currency') || 'ZAR';
};
...
export const updateCurrencyInAllComponents = (currency) => {
